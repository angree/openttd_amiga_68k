...
static bool CreateMainSurface(uint w, uint h, int want_backend)
{
	...
	if (want_backend == AMIGAGFX_BACKEND_EHB && !_amiga_ehb_active) {
		amigagfx_log("EHB mode selected - size applied now, 6 bitplanes on the next"
	...
	/* amigagfx_open falls back to a plain 8-bitplane AGA screen when RTG or EHB
	...
	_settings_client.amiga.rtg = (got_backend == AMIGAGFX_BACKEND_RTG);
	...
	DEBUG(driver, 1, "amiga: %dx%d, %s", w, h,
	      ...
	      got_backend == AMIGAGFX_BACKEND_EHB ? "6 bitplanes EHB, Kalms c2p1x1_6_c5_bm_040"
...
const char *VideoDriver_Amiga::Start(const char * const *parm)
{
	...
	{
		char b[96];
		snprintf(b, sizeof(b), "resolution list: %d AGA + %d EHB + %u RTG mode(s)",
	...
	if (_amiga_ehb_active) {
		amigagfx_log("EHB mode: 6 bitplanes, 64 colours - sprites reduced at load time");
	}
...
bool VideoDriver_Amiga::ChangeResolution(int w, int h)
{
	...
	if (!CreateMainSurface(w, h, _amiga_res_backend[index])) {
	...
	/* Record the EHB choice for the NEXT run. Deliberately taken from the list
	 * entry the player picked rather than from what just opened: mid-session
	 * CreateMainSurface refuses to open six bitplanes over 256-colour sprites,
	 * so the screen on display is not yet EHB - but the intent is, and a restart
	 * is exactly what makes it so. The note below says as much. */
	_settings_client.amiga.ehb =
		(_amiga_res_backend[index] == AMIGAGFX_BACKEND_EHB);
	...
	if ((_amiga_res_backend[index] == AMIGAGFX_BACKEND_EHB) != (_amiga_ehb_active != 0)) {
		amigagfx_log(_amiga_res_backend[index] == AMIGAGFX_BACKEND_EHB
		             ? "OCS/EHB: restart for 6 bitplanes and the 64-colour graphics"
		             : "AGA/RTG: restart for the full 256-colour graphics");
		ShowErrorMessage(INVALID_STRING_ID, STR_AMIGA_RESTART_FOR_MODE, 0, 0, true);
	}
	return true;
}
